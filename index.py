...
import settings
from my_db import Session, ComputerHardware, GroupMatch
from match import hardwarematches

app = application = bottle.Bottle()


def labelmatch(hardware):
    labels = []
    session = Session()
    t = session.query(GroupMatch).all()
    session.close()
    for i in t:
        if hardwarematches(hardware, i.match):
            labels.append(i.groupname)
...
def main():
    ...
    computers = computers.group_by(ComputerHardware.hostname).order_by(
        ComputerHardware.date.desc()
    )
    computers = computers.all()
    session.close()
    labels = {}
    for i in computers:
        labels[i.hostname] = labelmatch(i.hardware)
    return dict(computers=computers, labels=labels)
...
def computerview(name, version1=None):
    ...
    session = Session()
    computers = (
        session.query(ComputerHardware)
        .order_by(ComputerHardware.date.desc())
...
@app.route(
    settings.PREFIX + "/computer/<name>/<version1:int:[0-9]+>/<version2:int:[0-9]+>"
)
@view("diffview")
def diffview(name, version1, version2):
    ...
    if computer1 and computer2:
        ...
        abort(404, f"Компьютер {name} не найден")
    diff = jsondiff.diff(
        computer1.hardware, computer2.hardware, syntax="explicit", marshal=True
    )
    return dict(
        computer1=computer1,
        computer2=computer2,
...
def post_data():
    ...
    if computer:
        ...
        cdata = ComputerHardware(
            hostname=hostname, ip=ip_addr, date=datetime.datetime.now(), hardware=data
        )
        session.add(cdata)
    session.commit()
    session.close()
...
if __name__ == "__main__":
    bottle.run(app=app, debug=True, reloader=True, host="127.0.0.1", port=8888)
